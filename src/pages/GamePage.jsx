...
const GamePage = () => {
  ...
  useEffect(() => {
    fetch(
      `https://api.rawg.io/api/games/${id}?key=5bd48b5e817b487f8992f8046faaaae9`,
    )
      .then((res) => res.json())
      .then((data) => {
        setGameData(data);
  ...
  useEffect(() => {
    fetch(
      `https://api.rawg.io/api/games/${id}/movies?key=5bd48b5e817b487f8992f8046faaaae9`,
    )
      .then((res) => res.json())
      .then((data) => {
        setGameTrailer(data);
      })
      .then(() => console.log(gameTrailer))
      .catch((err) => {
        console.log("Err " + err);
      });
  }, [gameData]);
  ...
  return (
    <div className="bg-black">
      <Navbar />
      <div
        className={`w-screen h-screen text-white bg-no-repeat bg-cover`}
        style={{
          backgroundImage: gameData
      ...
      >
        ...
        <div className="text-5xl font-bold p-4 z-10">
          <span className="bg-[rgba(0,0,0,0.7)] py-2 px-4 rounded-xl">
            {gameData && gameData.name}
          </span>
          {gameData && (
            <div className="text-white w-1/2 bg-[rgba(0,0,0,0.5)] p-4 text-xl font-light mt-8 leading-9 rounded-lg backdrop-blur-sm">
